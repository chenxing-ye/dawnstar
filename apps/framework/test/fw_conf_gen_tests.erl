...
gen_test_impl() ->
    ...
    try
        ...
        % 生成配置模块
        {module, ?TEST_MODULE_NAME} = fw_conf_gen:gen(?TEST_MODULE_NAME, ?TEST_APP_NAME, 
...
set_test_impl() ->
    ...
    try
        ...
        % 测试应用名称不匹配的情况
        ?assertError({app_name_not_match, ?TEST_APP_NAME}, 
                     fw_conf_gen:set(?TEST_MODULE_NAME, wrong_app_name, test_key, test_value)),
        
        % 测试模块未初始化的情况
        code:purge(?TEST_MODULE_NAME),
        code:delete(?TEST_MODULE_NAME),
        ?assertError({app_config_not_inited, ?TEST_APP_NAME}, 
